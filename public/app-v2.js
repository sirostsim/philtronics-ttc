...
function renderHomeActiveJobs(timers) {
  ...
  timers.forEach(t=>{
    // Use server net elapsed (excludes paused time) — same source as wallboard and timer screen
    const referenceMs = t.isPaused && t.pausedAt
      ? new Date(t.pausedAt).getTime() : now;
...
function showMessageNotification(data) {
  ...
  try{const ctx=new(window.AudioContext||window.webkitAudioContext)(),osc=ctx.createOscillator(),gain=ctx.createGain();osc.connect(gain);gain.connect(ctx.destination);osc.frequency.value=880;gain.gain.setValueAtTime(0.15,ctx.currentTime);gain.gain.exponentialRampToValueAtTime(0.001,ctx.currentTime+0.4);osc.start(ctx.currentTime);osc.stop(ctx.currentTime+0.4);}catch(_){}
...
function openSendMessageModal(operatorId,operatorName) {
  ...
  textarea.addEventListener('input',()=>{charCount.textContent=textarea.value.length+' / 500';});body.appendChild(charCount);
  const errDiv=el('div',{className:'error-msg',role:'alert'});body.appendChild(errDiv);
  const btnSend=el('button',{className:'btn btn-primary',textContent:'\u2709 Send Message'});
  const btnCancel=el('button',{className:'btn btn-ghost',textContent:'Cancel'});
  btnCancel.addEventListener('click',closeModal);
  btnSend.addEventListener('click',async()=>{
    const message=textarea.value.trim();if(!message){errDiv.textContent='Please type a message.';return;}
    btnSend.disabled=true;btnSend.textContent='Sending\u2026';
    try{const result=await POST('/messages/send',{operatorId,message});closeModal();toast(result.delivered?'Message delivered to '+result.operatorName:result.operatorName+' is not currently logged in \u2014 message not delivered.',result.delivered?'success':'');}
