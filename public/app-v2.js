...
function navigateTo(page) {
  ...
  if (page !== 'wallboard')  { if (wallboardInterval)  { clearInterval(wallboardInterval);  wallboardInterval  = null; } if (wallboardTick)  { clearInterval(wallboardTick);  wallboardTick  = null; } }
  if (page !== 'wallboardc') { if (wallboardCInterval) { clearInterval(wallboardCInterval); wallboardCInterval = null; } if (wallboardCTick) { clearInterval(wallboardCTick); wallboardCTick = null; } }
  for (const p of Object.values(PAGES)) { const e = document.getElementById(p.id); if (e) e.hidden = true; }
  const target = PAGES[page]; if (target) { const node = document.getElementById(target.id); if (node) node.hidden = false; }
  buildNav();
...
function onLoggedIn() {
  ...
  if (hasRole('supervisor')) navigateTo('home'); else navigateTo('timer');
...
document.getElementById('btnTotpVerify').addEventListener('click', async () => {
  ...
  const btn = document.getElementById('btnTotpVerify'); btn.disabled = true; btn.textContent = 'Verifying\u2026';
  try {
...
function refreshActiveTimerBanner() {
  const banner = document.getElementById('activeTimerBanner'); banner.innerHTML = '';
  if (state.activeTimerId) {
    banner.appendChild(el('div', { className: 'active-banner-pill' },
...
async function loadTimerPage() {
  ...
  try {
    ...
    if (state.activeTimerId) { await showActivePanel(); startStopwatch(); } else showStartPanel();
  }
  loadTodayEntries();
...
function showStartPanel() {
  ...
  state.activePausedAt = null; state.activeTotalPausedSeconds = 0; state.activeHandRaised = false;
    ...
    if (state.activeTimerId) {
      try {
        ...
        if (direct && direct.status === 'active') t = direct;
          ...
          state.activeTimerId = null; state.activeStartedAt = null;
          refreshActiveTimerBanner(); showStartPanel(); stopStopwatch();
          toast('Your previous timer was already stopped.', ''); return;
        }
      } catch (_) {}
    }
    if (!t) { const timers = await GET('/timers?status=active'); t = timers.find(x => x.id === state.activeTimerId); }
    if (t) {
      ...
      updateActiveTargetDisplay(); updatePauseUI(); updateHandUI();
...
document.getElementById('btnStart').addEventListener('click', async () => {
  ...
  const btn = document.getElementById('btnStart'); btn.disabled = true;
  try {
    let timer;
    try {
      ...
      if (startErr.status === 409) {
        ...
        if (!document.getElementById('btnResumeTimer')) {
          ...
          resumeBtn.id = 'btnResumeTimer'; resumeBtn.className = 'btn btn-primary btn-full';
          resumeBtn.style.marginTop = '8px'; resumeBtn.textContent = '\u21a9 Resume My Active Timer';
          resumeBtn.addEventListener('click', async () => {
            ...
            if (me && me.activeTimer) {
              ...
              refreshActiveTimerBanner(); await showActivePanel(); startStopwatch();
            }
          });
    ...
    hideSuggestions(); showActivePanel(); startStopwatch();
    refreshActiveTimerBanner(); loadTodayEntries(); startPausePoll();
...
document.getElementById('btnStop').addEventListener('click', async () => {
  ...
  const btn = document.getElementById('btnStop'); btn.disabled = true;
  try {
...
document.getElementById('btnCancelTimer').addEventListener('click', () => {
  ...
  const errDiv = el('div', { className: 'error-msg', role: 'alert' }); bodyDiv.appendChild(errDiv);
  ...
  btnConfirm.addEventListener('click', async () => {
    ...
    try {
      ...
      state.activeTimerId = null; state.activeStartedAt = null; state.activeHandRaised = false;
      stopStopwatch(); showStartPanel(); refreshActiveTimerBanner(); loadTodayEntries();
...
function tickStopwatch() {
  ...
  const referenceMs = state.activeIsPaused && state.activePausedAt ? new Date(state.activePausedAt).getTime() : Date.now();
...
function updateActiveTargetDisplay(elapsed) {
  if (elapsed === undefined) {
    const refMs = state.activeIsPaused && state.activePausedAt ? new Date(state.activePausedAt).getTime() : Date.now();
    const raw = state.activeStartedAt ? Math.max(0, Math.floor((refMs - new Date(state.activeStartedAt).getTime()) / 1000)) : 0;
  ...
  if (!tgt || !wrap) return;
  wrap.hidden = false;
  const pct = elapsed / tgt, remaining = tgt - elapsed, over = remaining <= 0;
...
document.getElementById('btnDashSearch').addEventListener('click', async () => {
  ...
  const item = document.getElementById('dashItem').value.trim(), op = document.getElementById('dashOperator').value.trim();
  const params = new URLSearchParams();
  if (from) params.set('from', new Date(from).toISOString());
  if (to)   { const d = new Date(to); d.setHours(23,59,59,999); params.set('to', d.toISOString()); }
  if (item) params.set('itemNumber', item); if (op) params.set('operatorId', op);
...
function renderDashTable(rows) {
  const wrap = document.getElementById('dashTable'); wrap.innerHTML = '';
  if (!rows||!rows.length) { wrap.appendChild(el('div',{className:'empty-state',textContent:'No data for selected filters.'})); return; }
  const table = el('table');
  table.appendChild(el('thead',{},el('tr',{},el('th',{textContent:'Item Number'}),el('th',{textContent:'Count'}),el('th',{textContent:'Avg Actual'}),el('th',{textContent:'Min'}),el('th',{textContent:'Max'}),el('th',{textContent:'Target'}),el('th',{textContent:'Avg Delta'}))));
  const tbody = el('tbody',{});
  rows.forEach(r => {
    const hasTarget = r.target_seconds != null, delta = hasTarget ? Math.round(r.avg_seconds) - r.target_seconds : null;
    const row = el('tr',{},el('td',{textContent:r.item_number}),el('td',{textContent:r.count}),el('td',{textContent:formatDuration(Math.round(r.avg_seconds))}),el('td',{textContent:formatDuration(r.min_seconds)}),el('td',{textContent:formatDuration(r.max_seconds)}),el('td',{textContent:hasTarget?formatHM(r.target_seconds):'\u2014',className:hasTarget?'':'dash-no-target'}));
    const dc = el('td',{textContent:delta===null?'\u2014':(delta>=0?'+':'')+formatDuration(Math.abs(delta)),className:delta===null?'dash-no-target':delta>0?'dash-over':'dash-under'});
...
function renderAdminTools() {
  const btn = document.getElementById('btnCancelStuck'), resultDiv = document.getElementById('cancelStuckResult');
  if (!btn) return;
...
function renderUserList(users) {
  ...
  users.forEach(u => {
    ...
    if (ROLES_REQUIRING_TOTP.includes(u.role)) {
      const fa2Btn = el('button',{className:'btn btn-ghost',textContent:u.totpEnabled?'Reset 2FA':'2FA: Off',
        title:u.totpEnabled?'Reset this user\'s two-factor authentication':'2FA not yet configured',
...
function openUserModal(user) {
  const isNew = !user, body = el('div',{});
  [{id:'mUsername',label:'Username *',value:user?.username||'',disabled:!isNew},{id:'mFullName',label:'Full Name *',value:user?.fullName||''}].forEach(f=>{
  ...
  ['operator','supervisor','manager','administrator'].forEach(r=>{const o=el('option',{value:r,textContent:r.charAt(0).toUpperCase()+r.slice(1)});if(user?.role===r)o.selected=true;roleSelect.appendChild(o);});
  body.appendChild(el('div',{className:'form-group'},el('label',{for:'mRole',textContent:'Role *'}),roleSelect));
  if (!isNew){const chk=el('input',{type:'checkbox',id:'mActive'});if(user.isActive)chk.checked=true;body.appendChild(el('div',{className:'form-group',style:'flex-direction:row;align-items:center;gap:10px;'},chk,el('label',{for:'mActive',textContent:'Account Active'})));}
  ...
  btnSave.addEventListener('click',async()=>{
    errDiv.textContent='';
    const fullName=document.getElementById('mFullName').value.trim(),role=document.getElementById('mRole').value;
    if (!fullName){errDiv.textContent='Full name is required.';return;}
    btnSave.disabled=true;
    try{
      if(isNew){const username=document.getElementById('mUsername').value.trim(),password=document.getElementById('mPassword').value;if(!username){errDiv.textContent='Username is required.';btnSave.disabled=false;return;}if(password.length<8){errDiv.textContent='Password must be at least 8 characters.';btnSave.disabled=false;return;}await POST('/users',{username,password,full_name:fullName,role});toast('User created.','success');}
      else{await PATCH(`/users/${user.id}`,{full_name:fullName,role,is_active:document.getElementById('mActive').checked});toast('User updated.','success');}
...
function openResetPasswordModal(user) {
  ...
  const btnCancel=el('button',{className:'btn btn-ghost',textContent:'Cancel'});
  btnCancel.addEventListener('click',closeModal);
  btnSave.addEventListener('click',async()=>{const pw=pwInput.value;if(pw.length<8){errDiv.textContent='Password must be at least 8 characters.';return;}btnSave.disabled=true;try{await POST(`/users/${user.id}/reset-password`,{password:pw});toast('Password reset.','success');closeModal();}catch(err){errDiv.textContent=err.message;btnSave.disabled=false;}});
...
function confirmReset2FA(user) {
  ...
  const btnCancel=el('button',{className:'btn btn-ghost',textContent:'Cancel'});
  btnCancel.addEventListener('click',closeModal);
  btnConfirm.addEventListener('click',async()=>{btnConfirm.disabled=true;try{await api('DELETE','/totp/reset/'+user.id);toast('2FA reset for '+user.fullName+'. They will be prompted to set it up again.','success');closeModal();loadAdminPage();}catch(err){errDiv.textContent=err.message;btnConfirm.disabled=false;}});
...
function renderEntryList(containerId, timers, showOperator = false) {
  ...
  timers.forEach(t => {
    ...
    if (isAdmin){const delBtn=el('button',{className:'btn-delete-timer',textContent:'\uD83D\uDDD1',title:'Delete this timer record','aria-label':'Delete timer record for '+t.itemNumber});delBtn.addEventListener('click',()=>confirmDeleteTimer(t,card,containerId,timers));right.appendChild(delBtn);}
...
function confirmDeleteTimer(t, card, containerId) {
  ...
  [['Item: '+t.itemNumber,'font-family:var(--font-mono);color:var(--accent);margin-bottom:4px;'],['Operator: '+t.operatorName,''],['Started: '+formatLocalTime(t.startedAt),''],['Status: '+t.status,'']].forEach(([txt,sty])=>summary.appendChild(el('div',{textContent:txt,style:sty})));
  ...
  btnConfirm.addEventListener('click',async()=>{
    ...
    try{await api('DELETE','/timers/'+t.id);if(t.id===state.activeTimerId){state.activeTimerId=null;state.activeStartedAt=null;stopStopwatch();refreshActiveTimerBanner();}card.remove();const c=document.getElementById(containerId);if(c&&!c.children.length)c.appendChild(el('div',{className:'empty-state',textContent:'No records found.'}));closeModal();toast('Timer record deleted.','');}
...
itemInput.addEventListener('input', () => {
  clearTimeout(acDebounce); const q = itemInput.value.trim();
  if (q.length < 1) { hideSuggestions(); return; }
  acDebounce = setTimeout(() => fetchSuggestions(q), 200);
});
itemInput.addEventListener('keydown', e => {
  const items = sugList.querySelectorAll('li'); if (!items.length) return;
...
async function fetchSuggestions(q) { try { showSuggestions(await GET(`/items?q=${encodeURIComponent(q)}`)); } catch (_) {} }
function showSuggestions(items) {
  sugList.innerHTML = ''; if (!items.length){hideSuggestions();return;}
  items.forEach(item=>{const li=el('li',{role:'option',tabindex:'-1'});li.dataset.value=item.item_number;li.appendChild(el('span',{textContent:item.item_number}));if(item.description)li.appendChild(el('span',{className:'sug-desc',textContent:item.description}));li.addEventListener('mousedown',e=>{e.preventDefault();itemInput.value=item.item_number;hideSuggestions();itemInput.focus();});sugList.appendChild(li);});
...
const scanner = (() => {
  ...
  async function open(inputEl,mode) {
    ...
    }catch(err){if(err.name==='NotAllowedError')setStatus('Camera permission denied. Tap the camera icon in your browser address bar to allow access.','error');else if(err.name==='NotFoundError')setStatus('No camera found on this device.','error');else setStatus('Camera error: '+err.message,'error');}
  }
  function startScanLoop(){scanInterval=setInterval(async()=>{if(!active||!detector||video.readyState<2)return;try{const barcodes=await detector.detect(video);if(barcodes&&barcodes.length>0){const text=barcodes[0].rawValue.trim();if(targetMode==='item'){if(/^[A-Za-z0-9\-_]{1,40}$/.test(text))onScanSuccess(text);else{setStatus(`Read "${text}" \u2014 not a valid item number. Try again.`,'error');setTimeout(()=>{if(active)setStatus('Scanning \u2014 point at a barcode or QR code');},2000);}}else{if(text.length>0)onScanSuccess(text.slice(0,500));}}}catch(_){}},300);}
  function onScanSuccess(text){clearInterval(scanInterval);scanInterval=null;setStatus('\u2713 Scanned: '+text,'success');if(targetInput){if(targetMode==='notes'&&targetInput.value.trim())targetInput.value=targetInput.value.trimEnd()+' '+text;else targetInput.value=text;if(targetMode==='item')hideSuggestions();}setTimeout(()=>{close();if(targetInput)targetInput.focus();toast((targetMode==='notes'?'Note':'Item number')+' scanned: '+text,'success');},700);}
  function close(){active=false;overlay.hidden=true;clearInterval(scanInterval);scanInterval=null;if(stream){stream.getTracks().forEach(t=>t.stop());stream=null;}video.srcObject=null;detector=null;torchEnabled=false;torchBtn.hidden=true;torchBtn.textContent='\uD83D\uDD26 Torch';setStatus('Initialising camera\u2026');}
  function tryEnableTorch(){if(!stream)return;const track=stream.getVideoTracks()[0];if(!track)return;const caps=track.getCapabilities?track.getCapabilities():{};if(caps.torch){torchBtn.hidden=false;torchBtn.onclick=async()=>{torchEnabled=!torchEnabled;try{await track.applyConstraints({advanced:[{torch:torchEnabled}]});torchBtn.textContent=torchEnabled?'\uD83D\uDD26 Torch On':'\uD83D\uDD26 Torch';}catch(_){}};}}
...
async function refreshWallboard() {
  const container=document.getElementById('wallboardTiles'),countEl=document.getElementById('wallboardCount'),updatedEl=document.getElementById('wallboardUpdated');
  if (!container) return;
  try {
    ...
    timers.forEach(t => {
      ...
      if (t.handRaised) {
        ...
        if (hasRole('supervisor')) {
          ...
          lowerBtn.addEventListener('click', async e => {
            ...
            try{await POST('/timers/'+t.id+'/lower-hand',{});toast('Hand lowered for '+t.operatorName,'success');await refreshWallboard();}
      ...
      tile.appendChild(el('div',{className:'wb-elapsed',textContent:formatDuration(elapsed),'data-timerid':t.id,'data-startedat':t.startedAt,'data-pausedseconds':String(t.totalPausedSeconds||0),'data-ispaused':t.isPaused?'1':'0'}));
      ...
      if (t.targetSeconds) {
        ...
        targetWrap.appendChild(el('div',{className:'wb-target-label'+(remaining<=0?' overdue':''),textContent:'\uD83C\uDFAF Target: '+formatHM(t.targetSeconds)+'  \u2014  '+labelText,'data-startedat':t.startedAt,'data-targetseconds':String(t.targetSeconds)}));
        const bar=el('div',{className:'wb-target-bar'});
        bar.appendChild(el('div',{className:'wb-target-fill'+(pct>=1?' over':''),style:'width:'+Math.round(pctCapped*100)+'%','data-startedat':t.startedAt,'data-targetseconds':String(t.targetSeconds)}));
        targetWrap.appendChild(bar);tile.appendChild(targetWrap);
      }
      if (hasRole('supervisor')) {
        const btnRow=el('div',{className:'wb-btn-row'});
        const pauseBtn=el('button',{className:'wb-pause-btn'+(t.isPaused?' is-paused':''),textContent:t.isPaused?'\u25b6 Resume':'\u23f8 Pause','aria-label':(t.isPaused?'Resume':'Pause')+' timer for '+t.operatorName});
        ...
        btnRow.appendChild(el('button',{className:'wb-msg-btn',textContent:'\u2709 Message','aria-label':'Send message to '+t.operatorName,onclick:()=>openSendMessageModal(t.operatorId,t.operatorName)}));
...
function startWallboardTick() {
  ...
  wallboardTick=setInterval(()=>{
    ...
    document.querySelectorAll('.wb-elapsed[data-startedat]').forEach(node=>{
      const startedAt=node.getAttribute('data-startedat'),pausedSecs=parseInt(node.getAttribute('data-pausedseconds')||'0',10),isPaused=node.getAttribute('data-ispaused')==='1';
      if (!startedAt) return;
      const rawElapsed=Math.max(0,Math.floor((Date.now()-new Date(startedAt).getTime())/1000)),elapsed=Math.max(0,rawElapsed-pausedSecs);
      if (!isPaused) node.textContent=formatDuration(elapsed);
      const tile=node.closest('.wallboard-tile');if(!tile||isPaused)return;
      tile.classList.remove('tile-warning','tile-overdue');
      const fill=tile.querySelector('.wb-target-fill'),tgt=fill?parseInt(fill.getAttribute('data-targetseconds'),10):0;
      if(tgt){const pct=elapsed/tgt;if(pct>=1.0)tile.classList.add('tile-overdue');else if(pct>=0.8)tile.classList.add('tile-warning');fill.style.width=Math.round(Math.min(1,pct)*100)+'%';fill.classList.toggle('over',pct>=1);const lbl=tile.querySelector('.wb-target-label');if(lbl){const rem=tgt-elapsed;lbl.textContent='\uD83C\uDFAF Target: '+formatHM(tgt)+'  \u2014  '+(rem>0?formatHM(rem)+' remaining':formatHM(Math.abs(rem))+' overdue');lbl.className='wb-target-label'+(rem<=0?' overdue':'');}}
      else{if(elapsed>4*3600)tile.classList.add('tile-overdue');else if(elapsed>2*3600)tile.classList.add('tile-warning');}
    });
  },1000);
}
...
async function loadTargetTimes(containerId='targetTimesList') {
  ...
  try{renderTargetList(await GET('/targets'),containerId);}catch(_){container.innerHTML='<div class="empty-state">Could not load target times.</div>';}
}
function renderTargetList(targets,containerId='targetTimesList') {
  ...
  targets.forEach(t=>{const row=el('div',{className:'target-row'});const info=el('div',{className:'target-row-info'});info.appendChild(el('span',{className:'target-item-number',textContent:t.itemNumber}));info.appendChild(el('span',{className:'target-time-display',textContent:formatHM(t.totalSeconds)}));const actions=el('div',{className:'target-row-actions'});actions.appendChild(el('button',{className:'btn btn-ghost btn-sm',textContent:'Edit',onclick:()=>openTargetModal(t,containerId)}));actions.appendChild(el('button',{className:'btn btn-ghost btn-sm',textContent:'\uD83D\uDDD1',onclick:()=>confirmDeleteTarget(t,containerId)}));row.appendChild(info);row.appendChild(actions);container.appendChild(row);});
}
...
function openTargetModal(existing,containerId='targetTimesList') {
  ...
  if(isNew){const scanBtn=el('button',{className:'btn-scan',type:'button','aria-label':'Scan barcode into item number'});scanBtn.innerHTML=`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 7V5a2 2 0 0 1 2-2h2"/><path d="M17 3h2a2 2 0 0 1 2 2v2"/><path d="M21 17v2a2 2 0 0 1-2 2h-2"/><path d="M7 21H5a2 2 0 0 1-2-2v-2"/><rect x="7" y="7" width="3" height="3"/><rect x="14" y="7" width="3" height="3"/><rect x="7" y="14" width="3" height="3"/><rect x="14" y="14" width="3" height="3"/></svg> Scan`;scanBtn.addEventListener('click',()=>scanner.open(ttItemInput,'item'));itemInputRow.appendChild(scanBtn);}
  ...
  timeInputs.appendChild(hoursInput);timeInputs.appendChild(el('span',{textContent:'h',style:'margin:0 6px;color:var(--text2);font-weight:600;'}));timeInputs.appendChild(minsInput);timeInputs.appendChild(el('span',{textContent:'m',style:'margin:0 6px;color:var(--text2);font-weight:600;'}));
  ...
  const btnCancel=el('button',{className:'btn btn-ghost',textContent:'Cancel'});
  btnCancel.addEventListener('click',closeModal);
  btnSave.addEventListener('click',async()=>{errDiv.textContent='';const itemNumber=(document.getElementById('ttItemNumber').value||'').trim().toUpperCase();const hours=parseInt(document.getElementById('ttHours').value,10)||0;const minutes=parseInt(document.getElementById('ttMinutes').value,10)||0;if(!itemNumber){errDiv.textContent='Item Number is required.';return;}if(hours===0&&minutes===0){errDiv.textContent='Target time must be greater than zero.';return;}btnSave.disabled=true;try{await POST('/targets',{itemNumber,hours,minutes});toast((isNew?'Target time added':'Target time updated')+' for '+itemNumber,'success');closeModal();loadTargetTimes(containerId);if(containerId!=='targetTimesList')loadTargetTimes('targetTimesList');}catch(err){errDiv.textContent=err.message;}finally{btnSave.disabled=false;}});
...
function confirmDeleteTarget(t,containerId='targetTimesList') {
  ...
  btnCancel.addEventListener('click',closeModal);
  btnConfirm.addEventListener('click',async()=>{btnConfirm.disabled=true;try{await api('DELETE','/targets/'+encodeURIComponent(t.itemNumber));toast('Target time removed for '+t.itemNumber,'');closeModal();loadTargetTimes(containerId);if(containerId!=='targetTimesList')loadTargetTimes('targetTimesList');}catch(err){errDiv.textContent=err.message;btnConfirm.disabled=false;}});
...
async function openTotpSetupModal() {
  ...
  const qrWrap=el('div',{style:'text-align:center;padding:20px 0;'});qrWrap.appendChild(el('div',{textContent:'Generating QR code\u2026',style:'color:var(--text3);'}));body.appendChild(qrWrap);
  const codeGroup=el('div',{className:'form-group',style:'margin-top:8px;'});codeGroup.appendChild(el('label',{for:'setupTotpCode',textContent:'Enter code from app *'}));
  ...
  try{const setup=await POST('/totp/setup',{});qrWrap.innerHTML='';qrWrap.appendChild(el('img',{src:setup.qrDataUrl,alt:'QR code for authenticator app',style:'width:200px;height:200px;border-radius:8px;'}));qrWrap.appendChild(el('p',{textContent:"Can't scan? Enter this code manually: "+setup.secret,style:'font-size:11px;color:var(--text3);margin-top:8px;word-break:break-all;'}));codeInput.focus();}
  catch(err){qrWrap.innerHTML='';qrWrap.appendChild(el('p',{textContent:'Could not load QR code: '+err.message,style:'color:var(--red);'}));}
  btnEnable.addEventListener('click',async()=>{errDiv.textContent='';const code=codeInput.value.trim();if(!/^\d{6}$/.test(code)){errDiv.textContent='Please enter the 6-digit code from your authenticator app.';return;}btnEnable.disabled=true;try{await POST('/totp/confirm',{code});state.user.totpEnabled=true;closeModal();toast('Two-factor authentication enabled successfully.','success');}catch(err){errDiv.textContent=err.message;btnEnable.disabled=false;}});
}
...
function updatePauseUI() {
  const isPaused=state.activeIsPaused,banner=document.getElementById('pauseBanner'),pauseBtn=document.getElementById('btnPauseTimer'),
        label=document.getElementById('activeJobLabel'),stopwatch=document.getElementById('stopwatch'),panel=document.getElementById('panelActive');
...
document.getElementById('btnPauseTimer').addEventListener('click', async () => {
  ...
  try {
    if(state.activeIsPaused){const t=await POST('/pause/'+state.activeTimerId+'/resume',{});state.activeIsPaused=false;state.activePausedAt=null;state.activeTotalPausedSeconds=t.totalPausedSeconds||0;updatePauseUI();toast('Timer resumed.','success');}
    else{const t=await POST('/pause/'+state.activeTimerId+'/pause',{reason:'Manual pause'});state.activeIsPaused=true;state.activePausedAt=t.pausedAt;updatePauseUI();toast('Timer paused.','');}
...
function startPausePoll() {
  ...
  pausePollInterval=setInterval(async()=>{
    ...
    try{
      ...
      if(waspaused!==state.activeIsPaused){updatePauseUI();toast(state.activeIsPaused?'Your timer has been automatically paused outside working hours.':'Your timer has automatically resumed for the new working day.',state.activeIsPaused?'':'success');}
...
function renderHomeActiveJobs(timers) {
  ...
  const now=Date.now();
  timers.sort((a,b)=>{const elA=now-new Date(a.startedAt).getTime(),elB=now-new Date(b.startedAt).getTime();const ovA=a.targetSeconds?elA/1000>a.targetSeconds:elA>4*3600000;const ovB=b.targetSeconds?elB/1000>b.targetSeconds:elB>4*3600000;if(ovA!==ovB)return ovA?-1:1;return elB-elA;});
  const grid=el('div',{className:'home-active-grid'});
  timers.forEach(t=>{
    const referenceMs=t.isPaused&&t.pausedAt?new Date(t.pausedAt).getTime():now;
    ...
    timeInfo.appendChild(el('span',{className:'home-active-elapsed'+(isOver?' text-red':isWarning?' text-amber':''),textContent:formatDuration(elapsed)}));
    if(t.targetSeconds){const rem=t.targetSeconds-elapsed;timeInfo.appendChild(el('span',{className:'home-active-target'+(isOver?' text-red':''),textContent:isOver?'\u26a0 '+formatHM(Math.abs(rem))+' overdue':'\uD83C\uDFAF '+formatHM(rem)+' left'}));}
...
function renderHomeTodayStats(stats, activeTimers=[]) {
  ...
  if (hasRole('supervisor')) {
    ...
    if(raisedCount>0){
      ...
      lowerBtn.addEventListener('click',async()=>{lowerBtn.disabled=true;lowerBtn.textContent='Lowering\u2026';try{const result=await POST('/timers/lower-all-hands',{});toast(result.message,'success');loadHomePage();}catch(err){toast(err.message,'error');lowerBtn.disabled=false;lowerBtn.textContent='\u270b Lower All';}});
...
function renderHomeUsers(users) {
  ...
  const body=card.querySelector('.home-card-body');body.innerHTML='';
  const active=users.filter(u=>u.isActive),disabled=users.filter(u=>!u.isActive),need2fa=active.filter(u=>['manager','administrator'].includes(u.role)&&!u.totpEnabled);
  ...
  [{label:'Active Accounts',value:active.length,cls:''},{label:'Disabled',value:disabled.length,cls:disabled.length?'text-amber':''},{label:'2FA Not Configured',value:need2fa.length,cls:need2fa.length?'text-red':'text-green'}].forEach(s=>{
    const item=el('div',{className:'home-user-stat'});item.appendChild(el('span',{className:'home-user-stat-val '+s.cls,textContent:s.value}));item.appendChild(el('span',{className:'home-user-stat-lbl',textContent:s.label}));summary.appendChild(item);
  });
  body.appendChild(summary);
  if(need2fa.length){const warn=el('div',{className:'home-2fa-warn'});warn.appendChild(el('span',{textContent:'\u26a0 Users without 2FA: '}));warn.appendChild(el('span',{textContent:need2fa.map(u=>u.fullName).join(', '),style:'font-weight:600;'}));body.appendChild(warn);}
}
function renderHomeQuickActions() {
  ...
  [{label:'\uD83D\uDCCB Wall Board',page:'wallboard',role:'supervisor'},{label:'\uD83D\uDCFA Compact Board',page:'wallboardc',role:'supervisor'},
   {label:'\uD83D\uDCCA Dashboard',page:'dashboard',role:'manager'},{label:'\uD83D\uDCC8 Reports',page:'reports',role:'manager'},
   {label:'\uD83C\uDFAF Target Times',page:'targets',role:'manager'},{label:'\uD83D\uDD50 History',page:'history',role:'operator'},
   {label:'\uD83D\uDC65 User Management',page:'admin',role:'administrator'}]
  .filter(a=>hasRole(a.role)).forEach(a=>{body.appendChild(el('button',{className:'home-action-btn',textContent:a.label,onclick:()=>{navigateTo(a.page);closeNav();}}));});
}
...
async function refreshWallboardCompact() {
  const container=document.getElementById('wallboardCTiles'),countEl=document.getElementById('wallboardCCount'),updatedEl=document.getElementById('wallboardCUpdated');
  if(!container)return;
  try{
    const [timers,onlineData]=await Promise.all([GET('/timers?status=active&limit=200'),GET('/messages/online').catch(()=>({online:[]}))]);
    const onlineSet=new Set(onlineData.online||[]);
    if(countEl)countEl.textContent=timers.length+' active job'+(timers.length!==1?'s':'');
    if(updatedEl)updatedEl.textContent='Updated '+new Date().toLocaleTimeString('en-GB');
    container.innerHTML='';
    ...
    timers.forEach(t=>{
      ...
      if(!t.isPaused){if(t.targetSeconds){const pct=elapsed/t.targetSeconds;if(pct>=1.0)tile.classList.add('tile-overdue');else if(pct>=0.8)tile.classList.add('tile-warning');}else{if(elapsed>4*3600)tile.classList.add('tile-overdue');else if(elapsed>2*3600)tile.classList.add('tile-warning');}}
      const opRow=el('div',{className:'wb-operator-row'});
      opRow.appendChild(el('span',{className:'presence-dot '+(onlineSet.has(t.operatorId)?'online':'offline'),title:onlineSet.has(t.operatorId)?'Session active':'Not connected'}));
      opRow.appendChild(el('span',{textContent:t.operatorName}));
      tile.appendChild(opRow);
      ...
      tile.appendChild(el('div',{className:'wbc-elapsed',textContent:formatDuration(elapsed),'data-startedat':t.startedAt,'data-targetseconds':t.targetSeconds?String(t.targetSeconds):'','data-pausedseconds':String(t.totalPausedSeconds||0),'data-ispaused':t.isPaused?'1':'0'}));
      container.appendChild(tile);
    });
  ...
  switch (data.type) {
    ...
    case 'reply':
      if (chat.conversationId && data.conversationId && chatDrawer.hidden && data.conversationId === chat.conversationId) {
        chatDrawer.hidden = false; chatDrawer.style.display = ''; chatOverlay.hidden = false;
      }
...
function openChatDrawer(data) {
  ...
  chatHeaderSub.textContent  = chat.isSupervisor ? 'Tap \u2715 to close the conversation' : 'Reply below \u2014 your supervisor can see this';
  chatMessages.innerHTML = '';
  appendChatMessage(data, true);
  chatDrawer.hidden = false; chatDrawer.style.display = ''; chatOverlay.hidden = false;
  chatInput.value = ''; chatCharCount.textContent = '0 / 500';
...
function appendChatMessage(data, isOpening = false) {
  ...
  bbl.appendChild(el('div',{className:'chat-bubble-time',textContent:new Date(data.sentAt).toLocaleTimeString('en-GB',{timeZone:'Europe/London',hour:'2-digit',minute:'2-digit'})}));
  bubble.appendChild(bbl); chatMessages.appendChild(bubble); chatMessages.scrollTop=chatMessages.scrollHeight;
  if (!isOpening&&!isMine){chatDrawer.classList.add('chat-pulse');setTimeout(()=>chatDrawer.classList.remove('chat-pulse'),600);}
}

function handleConversationClosed(data) {
  if (data.conversationId !== chat.conversationId) return;
  chatMessages.appendChild(el('div',{className:'chat-system-msg',textContent:data.closedBy+' closed the conversation.'}));
  chatMessages.scrollTop=chatMessages.scrollHeight;
  chatInput.disabled=true; chatSendBtn.disabled=true;
  setTimeout(()=>{chat.conversationId=null;closeChatDrawer(false);},2500);
}
...
function closeChatDrawer(sendCloseSignal = true) {
  ...
  chatDrawer.hidden=true; chatOverlay.hidden=true; chatDrawer.style.display='none';
  chatInput.disabled=false; chatSendBtn.disabled=false; chatMessages.innerHTML='';
  if (sendCloseSignal&&chat.isSupervisor&&chat.conversationId) {
    POST('/messages/close',{conversationId:chat.conversationId}).catch(()=>{});
  }
  chat.conversationId=null; chat.isSupervisor=false; chat.otherName=null;
}

async function sendChatMessage() {
  const text=chatInput.value.trim();if(!text||!chat.conversationId)return;
  ...
  try {
    ...
    appendChatMessage({conversationId:chat.conversationId,from:state.user.fullName,fromId:state.user.id,fromRole:state.user.role,message:text,sentAt:new Date().toISOString()});
    chatInput.value='';chatCharCount.textContent='0 / 500';
...
chatInput.addEventListener('keydown',e=>{if(e.key==='Enter'&&(e.ctrlKey||e.metaKey)){e.preventDefault();sendChatMessage();}});
chatInput.addEventListener('input',()=>{chatCharCount.textContent=chatInput.value.length+' / 500';});
chatClose.addEventListener('click',()=>closeChatDrawer(true));
chatOverlay.addEventListener('click',()=>{if(!chat.isSupervisor)closeChatDrawer(false);});

async function openSendMessageModal(operatorId, operatorName) {
  const body=el('div',{});
  const msgArea=el('textarea',{id:'initMsgText',placeholder:'Type your opening message\u2026',maxlength:'500',rows:'4',style:'width:100%;background:var(--bg3);border:1px solid var(--border);border-radius:6px;color:var(--text);font-size:15px;padding:12px;resize:vertical;font-family:var(--font-body);'});
  body.appendChild(el('p',{textContent:'Start a conversation with '+operatorName+'. You can continue chatting until you close the conversation.',style:'margin-bottom:14px;font-size:14px;color:var(--text2);'}));
  ...
  btnSend.addEventListener('click',async()=>{
    const message=msgArea.value.trim();if(!message){errDiv.textContent='Please type a message.';return;}
    btnSend.disabled=true;btnSend.textContent='Starting\u2026';
    try{
      const result=await POST('/messages/send',{operatorId,message});closeModal();
      if(result.delivered){
        chat.conversationId=result.conversationId;chat.isSupervisor=true;chat.otherName=operatorName;
        chatHeaderName.textContent=operatorName;chatHeaderSub.textContent='Tap \u2715 to close the conversation';
        chatMessages.innerHTML='';
        appendChatMessage({conversationId:result.conversationId,from:state.user.fullName,fromId:state.user.id,fromRole:state.user.role,message,sentAt:new Date().toISOString()},true);
        chatDrawer.hidden=false;chatDrawer.style.display='';chatOverlay.hidden=false;
        setTimeout(()=>chatInput.focus(),80);
      }else{toast(operatorName+' is not currently logged in \u2014 message not delivered.','');}
    }catch(err){errDiv.textContent=err.message;btnSend.disabled=false;btnSend.textContent='\uD83D\uDCAC Start Conversation';}
  });
  msgArea.addEventListener('keydown',e=>{if(e.key==='Enter'&&(e.ctrlKey||e.metaKey))btnSend.click();});
...
function playPing(type) {
  try{
    ...
    osc.connect(gain);gain.connect(actx.destination);osc.frequency.value=type==='reply'?660:880;
    gain.gain.setValueAtTime(0.12,actx.currentTime);gain.gain.exponentialRampToValueAtTime(0.001,actx.currentTime+0.4);
...
const C = { green:'#38a169',red:'#e53e3e',amber:'#d97706',blue:'#4299e1',grid:'rgba(255,255,255,0.07)',text:'#a0aec0' };
...
function renderPendingCharts() {
  ...
  renderChartDailyTrend(trends);renderChartItemOnTime(byItem);renderChartOperator(operators);
}
...
function switchReportTab(btn, showId, hideId) {
  ...
  document.getElementById(showId).hidden=false;document.getElementById(hideId).hidden=true;
  const canvasMap={trendChart:()=>renderChartDailyTrend(_lastTrends),itemChart:()=>renderChartItemOnTime(_lastByItem),operatorChart:()=>renderChartOperator(_lastOperators)};
  if(canvasMap[showId])requestAnimationFrame(()=>setTimeout(canvasMap[showId],30));
}

function loadReportsPage() {
  const today=new Date().toISOString().slice(0,10),ago30=new Date(Date.now()-30*24*3600*1000).toISOString().slice(0,10);
  if(!document.getElementById('reportFrom').value){document.getElementById('reportFrom').value=ago30;document.getElementById('reportTo').value=today;}
  runReport();
...
async function runReport() {
  ...
  [['trendChart','trendTable'],['itemChart','itemTable'],['operatorChart','operatorTable']].forEach(([show,hide])=>{
    const s=document.getElementById(show),h=document.getElementById(hide);if(s)s.hidden=false;if(h)h.hidden=true;
  });
  document.querySelectorAll('.report-tab').forEach((t,i)=>t.classList.toggle('active',i%2===0));
  ...
  _lastTrends=trends;_lastByItem=stats?.byItem||[];_lastOperators=operators;
  renderReportTrendTable(trends);renderReportItemTable(stats?.byItem||[]);renderReportOperatorTable(operators);renderReportOverdue(overdue);
...
function renderChartDailyTrend(rows) {
  destroyChart('dailyTrend');const canvas=document.getElementById('chartDailyTrend');
  if(!canvas||!rows.length)return;
  canvas.width=600;canvas.height=300;
  const labels=rows.map(r=>new Date(r.day).toLocaleDateString('en-GB',{day:'2-digit',month:'short'}));
  const jobs=rows.map(r=>r.jobs_completed),overdue=rows.map(r=>r.overdue_count),avgMins=rows.map(r=>r.avg_seconds?+(r.avg_seconds/60).toFixed(1):0);
  _charts.dailyTrend=new Chart(canvas,{type:'bar',data:{labels,datasets:[
    ...
    {label:'Avg Time (mins)',data:avgMins,type:'line',borderColor:C.amber,backgroundColor:'transparent',pointBackgroundColor:C.amber,pointRadius:3,tension:0.3,yAxisID:'yMins'},
  ]},options:{responsive:false,interaction:{mode:'index',intersect:false},plugins:{legend:{labels:{color:C.text,font:CFONT}}},scales:{
    x:{ticks:{color:C.text,font:CFONT},grid:{color:C.grid}},
    yJobs:{ticks:{color:C.text,font:CFONT},grid:{color:C.grid},beginAtZero:true,title:{display:true,text:'Jobs',color:C.text,font:CFONT}},
    yMins:{position:'right',ticks:{color:C.amber,font:CFONT},grid:{drawOnChartArea:false},beginAtZero:true,title:{display:true,text:'Avg Mins',color:C.amber,font:CFONT}},
  }}});
}

function renderChartItemOnTime(rows) {
  destroyChart('itemOnTime');const canvas=document.getElementById('chartItemOnTime');if(!canvas)return;
  const withTarget=rows.filter(r=>r.target_seconds).slice(0,12);
  if(!withTarget.length){const w=canvas.closest('.report-chart-wrap');if(w)w.style.display='none';return;}
  const w2=canvas.closest('.report-chart-wrap');if(w2)w2.style.display='';
  canvas.width=600;canvas.height=300;
  const labels=withTarget.map(r=>r.item_number),over=withTarget.map(r=>Math.round(r.avg_seconds)>r.target_seconds?r.count:0),onTime=withTarget.map((r,i)=>r.count-over[i]);
  _charts.itemOnTime=new Chart(canvas,{type:'bar',data:{labels,datasets:[
  ...
  ]},options:{responsive:false,plugins:{legend:{labels:{color:C.text,font:CFONT}},tooltip:{mode:'index',intersect:false}},scales:{
    x:{stacked:true,ticks:{color:C.text,font:CFONT},grid:{color:C.grid}},
    y:{stacked:true,ticks:{color:C.text,font:CFONT},grid:{color:C.grid},beginAtZero:true,title:{display:true,text:'Jobs',color:C.text,font:CFONT}},
  }}});
}

function renderChartOperator(rows) {
  destroyChart('operator');const canvas=document.getElementById('chartOperator');if(!canvas||!rows.length)return;
  canvas.width=600;canvas.height=300;
  const labels=rows.map(r=>r.operator_name.split(' ')[0]),jobs=rows.map(r=>r.jobs_completed),overdue=rows.map(r=>r.overdue_count),avgMins=rows.map(r=>r.avg_seconds?+(r.avg_seconds/60).toFixed(1):0);
  _charts.operator=new Chart(canvas,{type:'bar',data:{labels,datasets:[
    {label:'Jobs Completed',data:jobs,backgroundColor:C.blue,borderRadius:4,yAxisID:'yJobs'},
    {label:'Over Target',data:overdue,backgroundColor:C.red,borderRadius:4,yAxisID:'yJobs'},
    {label:'Avg Time (mins)',data:avgMins,type:'line',borderColor:C.amber,backgroundColor:'transparent',pointBackgroundColor:C.amber,pointRadius:4,yAxisID:'yMins'},
  ]},options:{responsive:false,interaction:{mode:'index',intersect:false},plugins:{legend:{labels:{color:C.text,font:CFONT}}},scales:{
    x:{ticks:{color:C.text,font:CFONT},grid:{color:C.grid}},
    yJobs:{ticks:{color:C.text,font:CFONT},grid:{color:C.grid},beginAtZero:true,title:{display:true,text:'Jobs',color:C.text,font:CFONT}},
    yMins:{position:'right',ticks:{color:C.amber,font:CFONT},grid:{drawOnChartArea:false},beginAtZero:true,title:{display:true,text:'Avg Mins',color:C.amber,font:CFONT}},
...
function renderReportStatCards(stats) {
  const container=document.getElementById('reportStatCards');container.innerHTML='';if(!stats)return;
  const byItem=stats.byItem||[],totalJobs=byItem.reduce((s,r)=>s+r.count,0),withTarget=byItem.filter(r=>r.target_seconds),overdueItems=withTarget.filter(r=>Math.round(r.avg_seconds)>r.target_seconds);
  const onTimeRate=withTarget.length?Math.round(((withTarget.length-overdueItems.length)/withTarget.length)*100):null;
  [{label:'Jobs Completed',value:totalJobs},{label:'Item Types',value:byItem.length},{label:'On-Time Rate',value:onTimeRate!=null?onTimeRate+'%':'\u2014'},{label:'Items Over Target',value:overdueItems.length}]
  .forEach(c=>container.appendChild(el('div',{className:'stat-card'},el('div',{className:'stat-label',textContent:c.label}),el('div',{className:'stat-value',textContent:c.value}))));
}
function renderReportItemTable(rows) {
  const wrap=document.getElementById('reportItemTable');wrap.innerHTML='';
  if(!rows||!rows.length){wrap.appendChild(el('div',{className:'empty-state',textContent:'No completed jobs in this period.'}));return;}
  const table=el('table');
  table.appendChild(el('thead',{},el('tr',{},el('th',{textContent:'Item'}),el('th',{textContent:'Jobs'}),el('th',{textContent:'Avg Actual'}),el('th',{textContent:'Min'}),el('th',{textContent:'Max'}),el('th',{textContent:'Target'}),el('th',{textContent:'Avg Delta'}),el('th',{textContent:'Status'}))));
  const tbody=el('tbody',{});
  rows.forEach(r=>{const hasTarget=r.target_seconds!=null,delta=hasTarget?Math.round(r.avg_seconds)-r.target_seconds:null;
    tbody.appendChild(el('tr',{},el('td',{textContent:r.item_number,className:'perf-item'}),el('td',{textContent:r.count}),el('td',{textContent:formatDuration(Math.round(r.avg_seconds))}),el('td',{textContent:formatDuration(r.min_seconds)}),el('td',{textContent:formatDuration(r.max_seconds)}),el('td',{textContent:hasTarget?formatHM(r.target_seconds):'\u2014',className:hasTarget?'':'dash-no-target'}),el('td',{textContent:delta==null?'\u2014':(delta>=0?'+':'')+formatDuration(Math.abs(delta)),className:delta==null?'dash-no-target':delta>0?'dash-over':'dash-under'}),el('td',{textContent:delta==null?'\u2014':delta>0?'\u26a0 Over':'\u2713 On time',className:delta==null?'':delta>0?'dash-over':'dash-under'})));
  });
  table.appendChild(tbody);wrap.appendChild(table);
}
function renderReportOperatorTable(rows) {
  const wrap=document.getElementById('reportOperatorTable');wrap.innerHTML='';
  if(!rows||!rows.length){wrap.appendChild(el('div',{className:'empty-state',textContent:'No data for this period.'}));return;}
  const table=el('table');
  table.appendChild(el('thead',{},el('tr',{},el('th',{textContent:'Operator'}),el('th',{textContent:'Jobs'}),el('th',{textContent:'Avg Time'}),el('th',{textContent:'Fastest'}),el('th',{textContent:'Slowest'}),el('th',{textContent:'Over Target'}),el('th',{textContent:'Time Checks'}))));
  const tbody=el('tbody',{});
  rows.forEach(r=>{const overduePct=r.jobs_completed>0?Math.round((r.overdue_count/r.jobs_completed)*100):0;
    tbody.appendChild(el('tr',{},el('td',{textContent:r.operator_name,className:'perf-item'}),el('td',{textContent:r.jobs_completed}),el('td',{textContent:formatDuration(r.avg_seconds)}),el('td',{textContent:formatDuration(r.min_seconds)}),el('td',{textContent:formatDuration(r.max_seconds)}),el('td',{textContent:r.overdue_count+(overduePct?` (${overduePct}%)`:''),className:r.overdue_count>0?'dash-over':''}),el('td',{textContent:r.time_check_count})));
  });
  table.appendChild(tbody);wrap.appendChild(table);
}
function renderReportTrendTable(rows) {
  const wrap=document.getElementById('reportTrendTable');wrap.innerHTML='';
  if(!rows||!rows.length){wrap.appendChild(el('div',{className:'empty-state',textContent:'No data for this period.'}));return;}
  const table=el('table');
  table.appendChild(el('thead',{},el('tr',{},el('th',{textContent:'Date'}),el('th',{textContent:'Jobs Completed'}),el('th',{textContent:'Avg Time'}),el('th',{textContent:'Over Target'}))));
  const tbody=el('tbody',{});
  [...rows].reverse().forEach(r=>{const date=new Date(r.day).toLocaleDateString('en-GB',{weekday:'short',day:'2-digit',month:'short',year:'numeric'});
    tbody.appendChild(el('tr',{},el('td',{textContent:date}),el('td',{textContent:r.jobs_completed}),el('td',{textContent:r.avg_seconds?formatDuration(r.avg_seconds):'\u2014'}),el('td',{textContent:r.overdue_count,className:r.overdue_count>0?'dash-over':''})));
  });
  table.appendChild(tbody);wrap.appendChild(table);
}
function renderReportOverdue(overdue) {
  const grid=document.getElementById('reportOverdueGrid');grid.innerHTML='';
  ...
  if(!overdue.byItem||!overdue.byItem.length){itemCard.appendChild(el('div',{className:'empty-state',textContent:'No overdue jobs in this period.'}));}
  else{const table=el('table');table.appendChild(el('thead',{},el('tr',{},el('th',{textContent:'Item'}),el('th',{textContent:'Times Over'}),el('th',{textContent:'Avg Overrun'}),el('th',{textContent:'Worst Overrun'}))));const tbody=el('tbody',{});overdue.byItem.forEach(r=>{tbody.appendChild(el('tr',{},el('td',{textContent:r.item_number,className:'perf-item'}),el('td',{textContent:r.overdue_count,className:'dash-over'}),el('td',{textContent:'+'+formatDuration(r.avg_overrun_seconds)}),el('td',{textContent:'+'+formatDuration(r.max_overrun_seconds)})));});table.appendChild(tbody);itemCard.appendChild(table);}
  ...
  if(!overdue.byOperator||!overdue.byOperator.length){opCard.appendChild(el('div',{className:'empty-state',textContent:'No overdue jobs in this period.'}));}
  else{const table=el('table');table.appendChild(el('thead',{},el('tr',{},el('th',{textContent:'Operator'}),el('th',{textContent:'Times Over'}),el('th',{textContent:'Avg Overrun'}))));const tbody=el('tbody',{});overdue.byOperator.forEach(r=>{tbody.appendChild(el('tr',{},el('td',{textContent:r.operator_name,className:'perf-item'}),el('td',{textContent:r.overdue_count,className:'dash-over'}),el('td',{textContent:'+'+formatDuration(r.avg_overrun_seconds)})));});table.appendChild(tbody);opCard.appendChild(table);}
