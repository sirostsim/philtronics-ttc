...
function renderTargetList(targets, containerId = 'targetTimesList') {
  ...
  if (!targets || !targets.length) {
    container.appendChild(el('div', { className: 'empty-state', textContent: 'No target times set yet. Click + Add Target Time to get started.' })); return;
  }
  targets.forEach(t => {
...
async function openTotpSetupModal() {
  ...
  btnEnable.addEventListener('click', async () => {
    ...
    try {
      ...
      state.user.totpEnabled = true; closeModal();
  ...
  pausePollInterval = setInterval(async () => {
    ...
    try {
      ...
      if (waspaused !== state.activeIsPaused) {
        updatePauseUI();
        if (state.activeIsPaused) toast('Your timer has been automatically paused outside working hours.', '');
        else                      toast('Your timer has automatically resumed for the new working day.', 'success');
      }
...
function renderHomeActiveJobs(timers) {
  ...
  timers.forEach(t => {
    ...
    if (hasRole('supervisor')) {
      row.appendChild(el('button', { className: 'btn btn-ghost btn-sm home-msg-btn', textContent: '\u2709',
        title: 'Message ' + t.operatorName, onclick: () => openSendMessageModal(t.operatorId, t.operatorName) }));
    }
...
function renderHomeTodayStats(stats, activeTimers = []) {
  ...
  // Raised hands tile — shown to supervisors+
  if (hasRole('supervisor')) {
    ...
    if (raisedCount > 0) {
      ...
      lowerBtn.addEventListener('click', async () => {
        ...
        try {
          ...
          toast(result.message, 'success'); loadHomePage();
...
function renderHomePerformance(stats) {
  ...
  const body = card.querySelector('.home-card-body'); body.innerHTML = '';
  if (!stats || !stats.byItem || !stats.byItem.length) {
    body.appendChild(el('div', { className: 'empty-state', textContent: 'No completed jobs today.' })); return;
  }
  const table = el('table', { className: 'home-perf-table' });
  table.appendChild(el('thead', {}, el('tr', {},
    el('th', { textContent: 'Item' }), el('th', { textContent: 'Jobs' }),
    el('th', { textContent: 'Avg Time' }), el('th', { textContent: 'Target' }), el('th', { textContent: 'Delta' }))));
  const tbody = el('tbody', {});
  stats.byItem.slice(0, 10).forEach(r => {
    ...
    const tr = el('tr', {},
      ...
      el('td', { textContent: hasTarget ? formatHM(r.target_seconds) : '\u2014', className: hasTarget ? '' : 'dash-no-target' }));
    tr.appendChild(el('td', { textContent: delta === null ? '\u2014' : (delta >= 0 ? '+' : '') + formatDuration(Math.abs(delta)),
  ...
  body.appendChild(el('button', { className: 'btn btn-ghost btn-sm', textContent: '\u2b07 Export Today CSV', style: 'margin-top:12px;',
    onclick: () => { const today = new Date(); today.setHours(0,0,0,0); window.location.href = `/api/export/csv?from=${today.toISOString()}`; } }));
}
...
function renderHomeQuickActions() {
  ...
  actions.filter(a => hasRole(a.role)).forEach(a => {
    const btn = el('button', { className: 'home-action-btn', textContent: a.label, onclick: () => { navigateTo(a.page); closeNav(); } });
...
function handleConversationClosed(data) {
  ...
  const sys = el('div', { className: 'chat-system-msg', textContent: data.closedBy + ' closed the conversation.' });
...
function renderReportStatCards(stats) {
  const container = document.getElementById('reportStatCards'); container.innerHTML = '';
  if (!stats) return;
...
function renderReportOverdue(overdue) {
  ...
  if (!overdue.byItem || !overdue.byItem.length) {
    ...
    table.appendChild(tbody); itemCard.appendChild(table);
  }
  ...
  if (!overdue.byOperator || !overdue.byOperator.length) {
    ...
    table.appendChild(tbody); opCard.appendChild(table);
  }
