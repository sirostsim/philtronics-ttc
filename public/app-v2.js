  ...
  try {
    ...
    timers.forEach(t => {
      ...
      if (hasRole('supervisor')) {
        ...
        btnRow.appendChild(el('button',{className:'wb-msg-btn',textContent:'\u2709 Message',
  ...
  wallboardTick=setInterval(()=>{
    ...
    document.querySelectorAll('.wb-elapsed[data-startedat]').forEach(el=>{
      ...
      if (tgt){
        ...
        if(pct>=1.0)tile.classList.add('tile-overdue');else if(pct>=0.8)tile.classList.add('tile-warning');
        fill.style.width=Math.round(Math.min(1,pct)*100)+'%';fill.classList.toggle('over',pct>=1);
...
function startPausePoll() {
  ...
  pausePollInterval=setInterval(async()=>{
    ...
    try{
      ...
      state.activeIsPaused=t.isPaused||false;state.activePausedAt=t.pausedAt||null;state.activeTotalPausedSeconds=t.totalPausedSeconds||0;
...
async function openSendMessageModal(operatorId, operatorName) {
  ...
  const msgArea = el('textarea', {
    id: 'initMsgText', placeholder: 'Type your opening message\u2026',
