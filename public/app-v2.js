...
function renderDashTable(rows) {
  ...
  wrap.innerHTML = '';
  if (!rows || rows.length === 0) { wrap.appendChild(el('div', { className: 'empty-state', textContent: 'No data for selected filters.' })); return; }
...
function renderEntryList(containerId, timers, showOperator = false) {
  ...
  if (!container) return;
  container.innerHTML = '';
  if (!timers || timers.length === 0) { container.appendChild(el('div', { className: 'empty-state', textContent: 'No records found.' })); return; }
  ...
  try {
    ...
    timers.forEach(t => {
      ...
      if (t.targetSeconds) {
        ...
        targetWrap.appendChild(el('div', { className: 'wb-target-label', textContent: 'Target: ' + formatHM(t.targetSeconds) + '  (' + pct + '%)' }));
        const bar  = el('div', { className: 'wb-target-bar' });
        const fill = el('div', { className: 'wb-target-fill' + (pct >= 100 ? ' over' : ''), style: 'width:' + Math.min(pct, 100) + '%', 'data-startedat': t.startedAt, 'data-targetseconds': String(t.targetSeconds) });
...
async function loadTargetTimes() {
  ...
  try { const targets = await GET('/targets'); renderTargetList(targets); }
...
function renderTargetList(targets) {
  ...
  if (!targets || targets.length === 0) {
    container.appendChild(el('div', { className: 'empty-state', textContent: 'No target times set yet. Click + Add Target Time to get started.' }));
    return;
  }
  targets.forEach(t => {
    ...
    actions.appendChild(el('button', { className: 'btn btn-ghost btn-sm', textContent: 'Edit',    onclick: () => openTargetModal(t) }));
    actions.appendChild(el('button', { className: 'btn btn-ghost btn-sm', textContent: '🗑',      onclick: () => confirmDeleteTarget(t) }));
...
function openTargetModal(existing) {
  ...
  const itemInput = el('input', {
    id: 'ttItemNumber', type: 'text', maxlength: '40',
  ...
  body.appendChild(el('div', { className: 'form-group' },
    el('label', { for: 'ttItemNumber', textContent: 'Item Number *' }), itemInputRow));
